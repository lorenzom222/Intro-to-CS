...
public class Climber2 {
	...
	boolean keyLeftA, keyRightA, keyUpA;

	
	public Climber2(int x, int y, Game game) {
		this.game = game;
		this.x = x;
		this.y = y;
	...
	public void set () {
		...
		if(keyLeftA && keyRightA || !keyLeftA && !keyRightA) {
			xVelocity*=0.8;
		}
		else if(keyLeftA && !keyRightA) {
			xVelocity --;
		}
		else if(!keyLeftA && keyRightA) {
			xVelocity ++;
	...
	public void draw(Graphics2D create) {
		...
		create.setFont(f);
		create.drawString("Winner", 450, 100);
		create.drawString("Loser", 100, 100);
